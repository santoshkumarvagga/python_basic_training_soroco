...
class AboutInheritance(Koan):
    ...
    def test_subclasses_add_new_behavior(self):
        ...
        fido = self.Dog("Fido")
        with self.assertRaises(__): fido.wag()
    ...
    def test_super_works_across_methods(self):
        tiger = self.GreatDane("Tiger")
        self.assertEqual(__, tiger.growl())
    ...
    class Greyhound(Dog):
        # This is usually preferred way for subclasses
        def __init__(self, name):
            super().__init__(name)

    def test_base_init_does_not_get_called_automatically(self):
        snoopy = self.Pug("Snoopy")
        with self.assertRaises(__): name = snoopy.name
    ...
    def test_object_type(self):
        boxer = self.Greyhound("Boxer")
        # use isinstance in general, it will do subclass match
        self.assertEqual(__, isinstance(boxer, self.Greyhound))
        self.assertEqual(__, isinstance(boxer, self.Dog))

        # use type if you need strict match
        self.assertEqual(__, type(boxer) == self.Greyhound)
        self.assertEqual(__, type(boxer) == self.Dog)