...
class AboutClasses(Koan):
    ...
    def test_instances_of_classes_can_be_created_adding_parentheses(self):
        ...
        # use isinstance to check for object type
        self.assertEqual(__, isinstance(fido, self.Dog))

        # use type only if you do not want to compare with parent classes, learn more in inheritance koan
    ...
    def test_you_can_also_access_the_value_out_using_getattr_and_dict(self):
        ...
        # by method rather than through assignment operators. Do not use them unless you really need it

        # One use case is when you are not sure if the attribute exists
        with self.assertRaises(__):
            getattr(fido, "_breed")

        self.assertEqual(__, getattr(fido, "_breed", "Default"))
    ...
    def test_using_attributes_is_usually_enough(self):
        fido = self.Dog5()
