...
class AboutWithStatements(Koan):
    ...
    # ------------------------------------------------------------------
    # You can write context manager more easily as functions

    from contextlib import contextmanager
    @contextmanager
    def file_context_manager(self, file_name):
        file = open(file_name)
        yield file
        file.close()

    def count_lines3(self, file_name):
        with self.file_context_manager(file_name) as file:
            return len(file.readlines())

    def test_counting_lines3(self):
        self.assertEqual(__, self.count_lines3("example_file.txt"))
    # ------------------------------------------------------------------

    def count_lines4(self, file_name):
        with open(file_name) as file:
            return len(file.readlines())

    def test_open_already_has_its_own_built_in_context_manager(self):
        self.assertEqual(__, self.count_lines4("example_file.txt"))

    # Consider using context manager for any resources: Network connections, Application instances, DB connections